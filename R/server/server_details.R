detail_server <- function(id, all_data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$teamSpecificPickrates <- renderPlot({
      data <- team_pickrates()
      gen_data <- general_pickrates()
      ...
      if (!input$teamSpecificCompToggle) {
        data |>
          ggplot(aes(
          ...
          coord_flip() +
          labs(
            title = "Pickrates for Team",
            subtitle = "Average pickrates across all maps",
            x = "Hero",
            y = "Pickrate",
            caption = "Note: DPS and Support heroes appear more frequently as they occupy 2 slots per team, while Tank heroes only have 1 slot."
          ) +
    ...
    hero_preferences_team_specific <- reactive({
      team_win_rate <- filtered_data_team_specific() |>
        summarize(wr = mean(iswin, na.rm = TRUE)) |>
        pull(wr)

      hero_data <- team_pickrates() |>
        left_join(
          general_pickrates(),
          by = "hero_name",
          suffix = c("_team", "_general")
        ) |>
        mutate(
          winrate_deviation = avg_winrate_team * 100 - team_win_rate * 100,
          pickrate_deviation = weighted_pickrate_team - weighted_pickrate_general,
          category = case_when(
            (winrate_deviation < 0) & (pickrate_deviation < 0) ~ "Wisely Avoided",
            (winrate_deviation > 0) & (pickrate_deviation < 0) ~ "Underused Strength",
            (winrate_deviation < 0) & (pickrate_deviation > 0) ~ "Overused Weakness",
    ...
    output$teamSpecificQuadrants <- renderPlot({
      hero_data <- hero_preferences_team_specific()
      ...
      if (max(hero_data$maps_with_hero_total_team) > 20) {
        hero_data <- hero_data |>
          filter(maps_with_hero_total_team > 2)
      }
      ...
      hero_data |>
        ...
        labs(
          title = paste0(input$teamSpecificSelection, "Hero Performance"),
          subtitle = "Comparing pick rate and win rate relative to league averages",
          x = "Win Rate Deviation from League Average",
          y = "Pick Rate Deviation from League Average"
        ) +
